...
h = gr.plotMesh(ax(1),HK,'title','conductivities','fontsize',14); view(3); % show it

colorbar;

%% plotting conductivities in a cross section along x-axis and along y-axis
gr.plotXSec(1,'figure','xsec','title','Conductivities along y-axis','fontsize',14,'all','lay',HK);
colorbar;
...
gr.plotYSec(1,'figure','ysec','title','Conductivities along x-axis','fontsize',14,'all','lay',HK);
colorbar;
...
% view(3);
%%
animate = animateObj(basename,{'tracer','head','budget'});
animate.concYS(gr,'tracer',1);
animate.headYS(gr,1);
