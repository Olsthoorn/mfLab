...
clr='brgkmc';              % colors to plot lines
k=2;                  % [md ] conductivity (uniform)
Sy=0.1;               % [   ] specific yield
Ss=0.0005;            % [m-1] elastic specific storage coefficient
z0=0;                 % [NAP] z at xm(1)
D=100;                % [ m ] dummy aquifer thickness (because unconfined)
slope =-1/2;          % [m/m] dB/dz, inclination with B the bottom of the aquifer
Q0 = 5;               % [m2/d] discharge per m perpendicular to cross section

%% Specify grid line coordinates
dx=1;                                  % [ m ] grid cell width (uniform)
xGr=0:dx:1000;                         % [m] cell coordinates
...
A=40;                 % [ m ] set A=0 for constant slope
...
%% Arrays
IBOUND = gr.const(1); IBOUND(:,end,:)=-1; % Down-stream fixed head
...
STRTHD = gr.ZBlay+1;                         % [ m ] initial head 1 m above aquifer bottom
