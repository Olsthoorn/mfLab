...
I=find(xm>=-B/2 & xm<=B/2);	% Look for the x-nodes within the building pit
J=find(ym>=-L/2 & ym<=L/2);	% Same for y
...
if 1
r=25; theta=[0:15:360]'; xx=r*cos(theta)/pi; yy=r*sin(theta)/pi; zz=ones(size(theta)); tt=zeros(size(theta));
plist=[];
for j=1:length(j)
   plist=[plist;[xx yy zz*zm(J(j)) tt]];
end
end
