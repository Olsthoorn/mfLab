...
m=min(Phi(:)); M=max(Phi(:)); d=(M-m)/30; phirange=[m:d:M];										% stijghoogten
...
xlabel('r [m]'); ylabel('z [m]'); title('radblockctrd, Phi en Psix');
...
xlabel('r [m]'); ylabel('z [m]'); title('radblockctrd, Phi en Psiy');
set(gca,'ylim',[zBot,0],'xlim',[0,abs(zBot)]);
