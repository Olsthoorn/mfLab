...
kv=45000;		% WVP
k=sqrt(kh*kv); % mix
z=[0;-3.5;-53.5]; dz=abs(diff(z)); zm=0.5*(z(1:end-1)+z(2:end));
...
cS2=0.062;
KV1=KV;
KV1(1,ISm)=0.5*dz(1)/cS2;
[Phi1,Q1]=flatblockctrd(x,z,KH,KV1,FH,FQ);

figure
subplot(3,1,1); plot(xm,Phi1,'-xb');					set(gca,'xlim',xlim); grid on; title('onvolkomen, phi');
subplot(3,1,2); plot(x,[0,cumsum(Q1(1,:))],'-xb');	set(gca,'xlim',xlim); grid on; title('onvolkomen, q');
subplot(3,1,3); plot(xm,Q1(1,:)./diff(x),  '-xr');	set(gca,'xlim',xlim); grid on; title('onvolkomen, v');
qA1=sum(Q1(1,IAm),2);
qS1=sum(Q1(1,ISm),2);
