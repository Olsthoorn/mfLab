function [Phi,Q,QRF,QFF]=radblockctrd(r,z,kr,kz,FH,FQ)
% [Phi,Q,QRF,QFF]=RadBlockCtrd(r,z,kr,kz,FH,FQ)
...
if nargin==0,
	...
	z=[0:-5:-50]; 			Nz=length(z); dz=abs(diff(z));
   ka=25; kc=0.01;
   kr=ka*ones(Nz-1,Nr-1); kr(1,:)=kc;
	FH=NaN*zeros(Nz-1,Nr-1); FH(1,:)=0; FH(2:3,1)=-1;
	FQ=zeros(size(FH));
   [Phi,Q]=radblockctrd(r,z,kr,kr,FH,FQ);
   QW=sum(Q(1:end,1));
...
if nargout>2
   ...
   if z(end)<z(1),							% regular direction, y axis downward if rows numbered upard
      QFF=+diff(Phi,1,1).*ez;
   else
      QFF=-diff(Phi,1,1).*ez;
   end
