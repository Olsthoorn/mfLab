function POut=lineGrid(pline,varargin)  % xGr,yGr,zGr)
%LINEGRID get info on all line pieces of polyline pline intersecting a 1, 2 or 3D mesh
%
% Examples:
%   POut=linegrid(pline,xGr[,yGr[,zGr]]);
%   POut=linegrid(pline,gridObj);
%
...
%
%   alternatively use gridObj for a 3D grid
%
...
%
% Used by:
%     mflab/examples/swt_v4/Pennink/Series6/avi files
%
...
if strcmpi(class(varargin{1}),'gridObj')
    gr  = varargin{1};
    xGr = gr.xGr; Nx=gr.Nx;
    yGr = gr.yGr; Ny=gr.Ny;
    zGr = gr.Z;
    ...
    %varargin is expected to be [xGr [yGr [zGr]]] depending in 1D, 2D or 3dD grid
    nDim=min(3,nargin-1);  % 1 2 or 3
    
    switch nDim
        case 1, xGr=varargin{1}(:); 
        case 2, xGr=varargin{1}(:); yGr=varargin{2}(:); 
        case 3, xGr=varargin{1}(:); yGr=varargin{2}(:); zGr=varargin{3}(:);
        otherwise
            error('mfLab:linegrid:illegalDimensions',...
                'mfLab/%s dimension must be 1, 2 o3 3 not <<%d>>',nDim);
    end
end
