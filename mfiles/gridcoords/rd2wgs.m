...
%    lonlat     =rd2wgs(x,y)
%   [lon,lat]   =rd2wgs(x,y)
%   [lon,lat,LL]=rd2wgs(x,y); % LL is in string form
%
...
if nargout ==1,
    lamwgs = [lamwgs phiwgs];
end
   
if nargout>2
    N=length(x(:));
    LL=cell(N,1);  % allocate
    for i=1:length(phibes(:))
        LL{i,1}=sprintf('N %.7g, E %.7g',phiwgs(i),lamwgs(i));
    end
end

function [phi,lambda]=rd2bessel(x,y)
...
function [phiwgs,lamwgs]=bessel2wgs84(phibes, lambes)
...
x = [155000; 244000;  93000;  98000; 177000];
y = [463000; 601000; 464000; 471000; 439000];
