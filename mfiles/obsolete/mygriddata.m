function [xi,yi,zi] = mygriddata(x,y,z,xi,yi,method,options)
% MYGRIDDATA: Data gridding and surface fitting.
%   ADAPTED by OSTHOORN 090102 to tackle more layers at once
%   in case X and Y are vectors n long and Z ia an n by m matrix where m
%   is the number of layers to be interpolated.
%   This adaptation should be standard in matlab.
%
% TO 090102
%
...
%   $Revision: 5.33.4.8 $  $Date: 2007/06/14 05:11:18 $
...
if isvector(x) && isvector (y) && ~isvector(z)
    x=x(:); y=y(:);
    if length(x)~=size(z,1), z=z'; end
    Z=z;
    z=Z(:,1); % split of first column to not have to change gridddata
end
...
x = reshape(x,sz,1);
y = reshape(y,sz,1);
%z = reshape(z,sz,1);     % OLSTHOORN
sZ =numel(Z);             % OLSTHOORN
Z  = reshape(Z,sz,sZ/sz); % OLSTHOORN

%sxyz = sortrows([x y z],[2 1]); % OLSTHOORN
sxyz = sortrows([x y Z],[2 1]);  % OLSTHOORN

x = sxyz(:,1);
y = sxyz(:,2);
z = sxyz(:,3);
Z = sxyz(:,3:end);  % OLSTHOORN
...
if sum(ind) > 0
  ...
  for i = 1 : length(fs)
      for j=1:size(z,2)   % OLSTHOORN
        % averaging z values
    %   z(fe(i),j) = mean(z(fs(i):fe(i)));   % OLSTHOORN
        z(fe(i),j) = mean(z(fs(i):fe(i)));   % OLSTHOORN
      end                 % OLSTHOORN
  end
  x = x(~ind(2:end));
  y = y(~ind(2:end));
%  z = z(~ind(2:end));
  z = z(~ind(2:end),:);    % OLSTHOORN
end

switch lower(method),
  case 'linear'
%   zi = linear(x,y,z,xi,yi,opt);  % OLSTHOORN
    zi = linear(x,y,Z,xi,yi,opt);  % OLSTHOORN
  case 'cubic'
    if(isreal(z))
...
sz=size(z); % OLSTHOORN
zi=zeros(size(w,1),size(z,2));  % OLSTHOORN allocated
for j=1:sz(2)  % OLSTHOORN
    %z = z(:).'; % Treat z as a row so that code below involving
    zz = z(:,j).'; % OLSTHOORN
                % z(tri) works even when tri is 1-by-3.
                % z(tri) works even when tri is 1-by-3.
    %zi = sum(z(tri) .* w,2); OLSTHOORN
    zi(:,j) = sum(zz(tri) .* w,2); % OLSTHOORN
end % OLSTHOORN

%zi = reshape(zi,siz);  % OLSTHOORN, don't need reshape

%if ~isempty(out), zi(out)   = NaN; end   % OLSTHOORN
if ~isempty(out), zi(out,:) = NaN; end   % OLSTHOORN
%------------------------------------------------------------
